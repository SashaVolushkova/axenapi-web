...
class OpenAPITranslatorJsonTest {
    ...
    // Handle null specification input gracefully by returning null
    @Test
    ...
    public void test_spec_empty_only_name_and_version() throws OpenAPISpecParseException {
        EventGraphFacade facade = SolidOpenAPITranslator.parseOPenAPI("""
                {
                  "openapi": "3.0.0",
                  "info": {
                    "title": "Test Service",
        ...
        assertEquals("Test Service", result.getNodes().getFirst().getName());
        assertEquals(0, result.getLinks().size());
        assertEquals(0, result.getEvents().size());
        assertEquals("Test Service", result.getName());
    }

    // Test undefined broker type scenario where service accepts messages from unknown broker
    @Test
    public void test_parse_openapi_specification_with_undefined_broker() throws OpenAPISpecParseException {
        String specification = """
                {
                  "openapi": "3.0.0",
                  "info": {
                    "title": "Undefined Broker Service",
                    "version": "1.0.0"
                  },
                  "paths": {
        ...
        EventGraphDTO result = facade != null ? facade.eventGraph() : null;
        assertNotNull(result);
        assertEquals("Undefined Broker Service", result.getName());
        assertEquals(2, result.getNodes().size());
        assertEquals(1, result.getLinks().size());
        assertEquals(1, result.getEvents().size());
        ...
        NodeDTO serviceNode = result.getNodes().stream()
                ...
                .orElse(null);
        assertNotNull(serviceNode);
        assertEquals("Undefined Broker Service", serviceNode.getName());
        ...
        // Verify the link exists from topic to service (incoming message)
        LinkDTO link = result.getLinks().get(0);
        assertNotNull(link);
        assertEquals(topicNode.getId(), link.getFromId());
        assertEquals(serviceNode.getId(), link.getToId());
    }

    @Test
    public void test_parse_openapi_specification_with_undefined() throws OpenAPISpecParseException {
        String specification = """
                {
                  "openapi": "3.0.0",
                  "info": {
